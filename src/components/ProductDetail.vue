        ...
        <div class="flex flex-col font-mono">
            ...
            <div class="flex mt-3 gap-4">
                <button v-for="productVariant in productDetail.productVariants" :key="productVariant.id"
                    :style="{ backgroundColor: productVariant.colorHex }"
                    class="w-8 h-8 rounded-full bg-red-400 ring-2 ring-primary ring-offset-2 cursor-pointer"></button>
            </div>
            ...
            <div class="grid grid-cols-4 gap-2 mt-2">
                <button v-for="productVariant in sortedVariants" :key="productVariant.id"
                    class="py-2 px-12 border font-bold border-slate-200 text-sm rounded-lg hover:border-primary transition-all cursor-pointer">
                    {{ productVariant.size }}
                </button>
                <!-- <button
            ...
            <input type="number" value="1" class="w-20 p-2 mt-2 rounded-2xl outline-1 outline-gray-400" />
            <div class="flex gap-4 mt-4">
                <button class="flex-1 p-3.5 border-2 uppercase rounded-xl text-xl cursor-pointer font-bold">Thêm vào giỏ
                    hàng</button>
                <button
