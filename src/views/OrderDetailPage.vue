<template>
    <AccountManageSection>
        ...
        <div v-else class="p-8 bg-gray-50 flex-1 rounded-3xl">
            <div class="flex justify-between items-start mb-8">
                ...
                <button
                    class="flex items-center space-x-2 px-6 py-2 border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 shadow-sm">
                    <i class="pi pi-print"></i> <span>In hóa đơn</span>
                </button>
            </div>
            ...
            <div class="flex gap-8">
                <div class="flex-1 flex flex-col gap-8">
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 class="text-xl font-bold mb-6">Sản phẩm ({{ order.items?.length || 0 }})</h3>
                        <div class="space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                        <div class="flex justify-between"><span>Tạm tính</span><span>{{ formatCurrency(order.subtotal)
                                }}</span></div>
                        <div class="flex justify-between"><span>Phí vận chuyển</span><span>{{
                                formatCurrency(order.shippingFee) }}</span></div>
                        <div class="flex justify-between text-red-600"><span>Giảm giá</span><span>-{{
                                formatCurrency(order.discount) }}</span></div>
                        <div class="border-t pt-5 flex justify-between items-end">
                            <span class="text-lg font-bold">Tổng thanh toán</span>
                            <span class="text-3xl font-extrabold text-red-600">{{ formatCurrency(order.total) }}</span>
                        </div>
                    </div>
                ...
                <div class="w-96 flex flex-col gap-8">
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 class="text-xl font-bold mb-4 flex items-center gap-2">
                            <i class="pi pi-map-marker text-red-600"></i> Địa chỉ nhận hàng
                        </h3>
                        <p class="font-bold">{{ order.receiverName }}</p>
                        <p>{{ order.receiverPhone }}</p>
                        <p class="mt-4 text-gray-600 leading-relaxed">{{ order.receiverAddress }}</p>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div class="bg-gray-100 p-4 rounded-xl flex items-center gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ order.paymentInfo }}</p>
                            </div>
                        </div>
...
const steps = computed(() => {
  const currentStatus = props.order?.status || 'pending'
  const allSteps = [
